async function build() {
    ...
    function adjacencyMatrix(nodes2, nodes, edges) {
        var matrix = [];
        var edgeHash = {};
        edges.forEach(edge => {
    ...
    var data = adjacencyMatrix(nodes2, nodes, edges);
    console.table(data);
    const pole = bounds
        .selectAll("rect")
        .data(data)
    ...
    const namesX2 = wrapper
        .append("g")
        .attr("transform", `translate(${dimension.margin.left},${dimension.margin.top - 180})`)
        .selectAll("text")
        .data(nodes)
        .enter()
        .append("text")
    ...
    const namesY2 = wrapper
        .append("g")
        .attr("transform", `translate(${dimension.margin.left - 410},${dimension.margin.top})`)
        .selectAll("text")
        .data(nodes2)
        .enter()
        .append("text")
